...
//	}
	...
	@Override
	public ResponseDto register(UserDto userDto) {
	    // 1. Check Email Uniqueness (Always required)
	    if (!userDao.isEmailUnique(userDto.getEmail())) {
	        throw new DataExistsException("Email Already Exists: " + userDto.getEmail());
	    }

	    // 2. FIXED: Check Mobile Uniqueness ONLY if mobile is not null and not 0
	    // Since it's a Long/long, we check for null or a non-zero value
	    if (userDto.getMobile() != null && userDto.getMobile() != 0) {
	        if (!userDao.isMobileUnique(userDto.getMobile())) { 
	            throw new DataExistsException("Mobile Already Exists: " + userDto.getMobile());
	        }
	    }

	    // 3. Generate OTP and Save
	    int otp = new SecureRandom().nextInt(100000, 1000000);
	    emailSender.sendOtp(userDto.getEmail(), otp, userDto.getName());
	    
	    // Clean role string and save
	    String roleStr = userDto.getRole().toUpperCase().replace("ROLE_", "");
	    
	    userDao.saveUser(
	            new User(null, userDto.getName(), userDto.getEmail(), encoder.encode(userDto.getPassword()),
	                    userDto.getMobile(), null, otp, LocalDateTime.now().plusMinutes(5),
	                    Role.valueOf("ROLE_" + roleStr), false, 0, null));
